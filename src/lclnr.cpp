...
float b[30],rad;
int bbc=1;
//int alpha=1,beta=0,gama=1,delta=1,zeta=1;
int alpha=1,beta=1,gama=1,delta=1,zeta=1;
...
//int wlimit=20; //wlimit is the no of wavelengths available
int wlimit;
...
int main()
{
   ...
   fprintf(fplot,"traffic    blockprob  meanlinkut  avg_phoplth  syd. deviation\n");

  //for(meania=6;meania<20;meania+=2){  /* this is to be enabled for fixed load and variable converters run*/
 // for(meania=1;meania<24;meania+=2){/* arrival rates changed to change traffic; this to be enabled for fixed converter and variable load run*/
   ...
   for(meania=2;meania<21;meania+=2){
   fprintf(flog," \n");
   fprintf(flog,"*********************************\n");
    ...
    { for(j=0;j<n;j++)
     ...
     if(l[i][j][0]>0)
        nodedeg[i]++;
         ...
         printf("%d   ",nodedeg[i]);  }
convert=8;//this to be enabled for fixed load and variable converters run
  // convert=8;//this to be enabled for fixed converter and variable load run
   for(i=0;i<n;i++)
       converter[i]=convert;

   seed=8000+5*meania; /* seed fpr random no. generation */
   srand(seed);
     ...
     for(i=0;i<3;i++)
       ...
       /*   for(k=0;k<2;k++)
         ...
         //  for(j=0;j<=totalpath[i].bkpath[0].hoplen;j++)
           // printf(totalpath[i].bkpath[0].pth[j]);
         for(x=0;x<n;x++)
         for(y=0;y<n;y++)
            c[x][y][0]=l[x][y][0];
...
{       float mincost=1000,bw=0.25,mean,variance,g,wl[33];
    int dpath[40],free,wu,m,w_used[10][33],wt[10][10],comb[3],pp=0,bb=0,wb_used[10][15][33],pathdeg[3],bpathdeg[2][3];
  ...
  for(i=0;i<3;i++)
    { ruts=totalpath[i].primary;
        ...
        for(j=0;j<=ruts.hoplen;j++)
                pathdeg[i]+=nodedeg[ruts.pth[j]];
                printf("path degree is=%d",pathdeg[i]);
     ...
     // for(j=0;j<=ruts.hoplen;j++)
      ...
      for(j=0;j<ruts.hoplen;j++)
       ...
       // if(wavelnp+wavelnb==wlimit)
          ...
          for(j=0;j<ruts.hoplen;j++)
             {
                  variance=variance+fabs(wl[j]*wl[j]-mean*mean);
                  wavelnp=0;
                  wavelnb=0;
             }
             variance=variance/j;
     ...
     //   printf("\n mean and variance are is %f\n%f",mean,variance);
        g=alpha*mean+beta*variance+gama*j+delta*conv+zeta*pathdeg[i];
     //   printf("\ng is %f\n",g);
       
          totalpath[i].primary.weight=g;
         if(totalpath[i].primary.weight>1000)
        pp++;
 ...
 for(t=0;t<2;t++)
     {  totalpath[i].bkpath[t].weight=0;
        ruts=totalpath[i].bkpath[t];
         ...
         bpathdeg[i][t]=0;
           for(j=0;j<=ruts.hoplen;j++)
                bpathdeg[i][t]+=nodedeg[ruts.pth[j]];
                printf("path degree is=%d",bpathdeg[i][t]);
   ...
   //      printf("\n no. of wavelengths converters required are %d",conv);
    ...
    printf("\n mean and variance are is %f\n%f",mean,variance);
        g=alpha*mean+beta*variance+gama*j+delta*conv+zeta*bpathdeg[i][t];
  //      printf("\ng is %f\n",g);
         conv=0;
         mean=0;
         variance=0;
...
if(pp==3)
...
else if(bb==6||flag==6)
    {
         for(i=0;i<3;i++)
             {
                 if(totalpath[i].primary.weight<mincost)
                      {
                           mincost=totalpath[i].primary.weight;
                           chosenroute=i;
...
else
 {
     for(i=0;i<3;i++)
         for(k=0;k<2;k++)
             {
                 if(totalpath[i].totalweight[k]<mincost)
                      {
                           mincost=totalpath[i].totalweight[k];
                           chosenroute=i;
                           backroute=k;
                           }
                  else if(totalpath[i].totalweight[k]==mincost)
                        {
                             if(totalpath[i].primary.weight<totalpath[chosenroute].primary.weight)
                                {  chosenroute=i;
                                   backroute=k; }
                   }        }
